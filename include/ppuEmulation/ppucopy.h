...
class ppu
{
    ...
    public:
        ...
        static void clock(LTE::gameObject *eventEntity, LTE::coreEventData *sendor)
        {
            ...
            if (ppuData->scanline >= -1 && ppuData->scanline < 240)
            {		
                ...
                if (ppuData->cycle == 257)
                {
                    LoadBackgroundShifters();
                    TransferAddressX();
                }

                if (ppuData->cycle == 338 || ppuData->cycle == 340)
                {
                    ppuData->bg_next_tile_id = ppuData->ppuBus.read(0x2000 | (ppuData->vram_addr.reg & 0x0FFF));
                }

                if (ppuData->scanline == -1 && ppuData->cycle >= 280 && ppuData->cycle < 305)
                {
                    TransferAddressY();
                }
            }
            ...
            if (ppuData->scanline >= 241 && ppuData->scanline < 261)
            {
