...
{
    loadGameEvent *gameInfo = static_cast<loadGameEvent*>(sendor);
...
int main(int argc, char** argv) 
{
    ...
    LAUGHTALE_ENGINR_LOG_INFO( argc)

    if(argc >= 2)
    {
        loadGameEvent *sendorData = new loadGameEvent("load game/");
        std::string gameName = "./res/roms/";
        for (size_t i = 1; i < argc; i++)
        {
            gameName += std::string(argv[i]) + " ";
            /* code */
        }
        gameName = gameName.erase(gameName.length() - 1);
        sendorData->gamePath =  gameName + ".nes";
        LAUGHTALE_ENGINR_LOG_INFO( sendorData->gamePath)
